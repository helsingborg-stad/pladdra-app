...
namespace Pladdra
{
    ...
    public class RaycastManager : MonoBehaviour
    {
        ...
        [Tooltip("Screen Y limit below which raycasts are not sent.")]
        public float minTouchLimit = 100;
        [Tooltip("Screen X limit above which raycasts are not sent.")]
        ...
        void Update()
        {
            ...
            if (GetTouch(out touches))
            {
                ...
                if (mapTouch_Bottom != 0 && mapTouch_Top != 0)
                {
                ...
                // Ray rayOrigin = cam.ScreenPointToRay(screenPoint); //This one is for the viz LineRenderer
                // if (visualizeTouch)
                // {
        ...
        public void AddNamedLayerMask(string name, LayerMask layerMask)
        {
            layerMasks.Add(name, layerMask);
        }
